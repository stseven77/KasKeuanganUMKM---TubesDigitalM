<?php
// Tampilkan error PHP untuk debugging di server InfinityFree
ini_set('display_errors', 1);
ini_set('display_startup_errors', 1);
error_reporting(E_ALL);

// Memanggil logika PHP dari folder process
require_once __DIR__ . '/process/login-process.php';

// Tangkap parameter 'email' dari URL (Quick Signup)
...
<body class="bg-gradient-to-br from-indigo-50 via-purple-50 to-white dark:from-slate-950 dark:via-slate-900 dark:to-slate-900 text-slate-800 dark:text-slate-100 antialiased min-h-screen flex items-center justify-center p-4 sm:p-6 transition-colors duration-300">
    ...
    <div class="w-full max-w-md bg-white/85 dark:bg-slate-900/90 backdrop-blur-xl border border-indigo-100 dark:border-slate-800 shadow-2xl shadow-indigo-100/50 dark:shadow-none rounded-3xl p-8 sm:p-10 relative overflow-hidden transition-all duration-500">
        ...
        <div class="text-center mb-8 relative z-10">
            ...
            $baseUrl = '../../'; 
            ?>
            <a href="<?php echo $baseUrl; ?>index.html" class="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-white dark:bg-slate-800 border border-indigo-100 dark:border-slate-700 shadow-lg shadow-indigo-200/50 dark:shadow-none mb-4 hover:scale-105 transition-transform overflow-hidden p-2">
                <img src="<?php echo $baseUrl; ?>assets/img/logo/rinfisunguawal.png" alt="Logo RINFIS" class="w-full h-full object-contain">
            </a>
        ...
        <!-- Form Autentikasi -->
        <form action="login.php<?php echo !empty($passed_email) ? '?email=' . urlencode($passed_email) : ''; ?>" method="POST" class="space-y-4 relative z-10">
            ...
            <div class="space-y-1.5">
                ...
                <div class="relative">
                    ...
                    <input type="email" name="email" id="email" required value="<?php echo htmlspecialchars($passed_email); ?>" placeholder="nama@email.com" class="w-full pl-11 pr-4 py-3.5 rounded-xl bg-indigo-50/40 dark:bg-slate-800 border border-indigo-100 dark:border-slate-700 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:bg-white dark:focus:bg-slate-800 transition">
                </div>
            </div>
    ...
    <script>
        ...
        // Otomatis pindah ke Tab Register jika user datang dari Quick Signup
